...
showToastMessage(config: object) {
  ...
  onInit() {
    ...
    errorMessage(data:any,row:any){
      const lines = data.split(/\s*\.\s*/).filter(Boolean);
      const spanHTML = lines.map((line: any) => `<span>${line}</span>`).join('<br>');
      const customHTML = `<div>${spanHTML}</div>`;
